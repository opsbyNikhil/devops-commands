...
const STYLES = `
  ...
  @keyframes np-label-glow {
    0%, 100% { box-shadow: 0 0 0 0 rgba(245,158,11,0.4); }
    50%       { box-shadow: 0 0 0 4px rgba(245,158,11,0);  }
  }
  .np-flow-line {
    stroke-dasharray: 6 4;
    animation: np-dash-flow 0.65s linear infinite;
  }
  .np-ext-flow-line {
    stroke-dasharray: 8 6;
    animation: np-dash-flow 0.8s linear infinite;
  }
  .np-label-badge {
    animation: np-label-glow 2.8s ease-in-out infinite;
  }
`;
...
export default function NodePort() {
  ...
  const clusterBdr = isDark ? "rgba(245,158,11,0.28)" : "rgba(245,158,11,0.35)";

  // NodePort uses Amber/Orange as defined in eksMeta
  const svcAccent = "#f59e0b";
  const svcDim = isDark ? "rgba(245,158,11,0.15)" : "rgba(245,158,11,0.09)";
  const svcBorder = isDark ? "rgba(245,158,11,0.42)" : "rgba(245,158,11,0.32)";
  ...
  const extAccent = "#ec4899"; // Pink for external traffic
  const extDim = isDark ? "rgba(236,72,153,0.15)" : "rgba(236,72,153,0.09)";
  const extBorder = isDark ? "rgba(236,72,153,0.4)" : "rgba(236,72,153,0.32)";
  ...
  const POD_GAP = 12;
  const pods = [
    { name: "payment-app-1", container: "pay-image-1" },
    { name: "payment-app-2", container: "pay-image-2" },
    { name: "payment-app-3", container: "pay-image-3" },
    { name: "payment-app-4", container: "pay-image-4" },
  ];
  ...
  const SVG_INT_W = 90;
  const SVG_EXT_W = 60;
  const lineAccents = ["#f59e0b", "#326CE5", "#ec4899", "#22c55e"];
  ...
  return (
    <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "0 0 60px" }}>
      ...
      >
        A <span style={{ color: svcAccent }}>NodePort</span> Service exposes the
        Service on each Node's IP at a static port (the NodePort). It allows
        external traffic to reach your pods by requesting{" "}
        <code style={{ color: extAccent, fontSize: 10 }}>
          &lt;NodeIP&gt;:&lt;NodePort&gt;
        </code>
        .
      </p>
      ...
      >
          ...
          >
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              ...
              <ellipse
                ...
                strokeWidth="1.5"
              />
              <path d="M2 12h20" stroke={extAccent} strokeWidth="1.5" />
            </svg>
          </div>
          <p
            style={{
        ...
        {/* ── External SVG Connection ── */}
        <svg
          width={SVG_EXT_W}
          height={totalH + 60} // Taller to match the Node wrapper height
        ...
        >
          <g transform={`translate(0, ${(totalH + 60) / 2})`}>
            ...
            <path
              ...
              fill="none"
              strokeOpacity={0.8}
              className="np-ext-flow-line"
            />
            {/* Traveling packet dot */}
            <circle r={3.5} fill={extAccent} opacity={0.95}>
              <animateMotion
                dur="1.5s"
        ...
        {[
          {
            ...
            desc: "Kubernetes allocates port 30008 on every Node in the cluster.",
            accent: extAccent,
            dim: extDim,
            border: extBorder,
          },
          {
            ...
            desc: "Traffic from the outside hits the Node's IP address on port 30008.",
            accent: svcAccent,
            dim: svcDim,
            border: svcBorder,
          },
          {
            ...
            desc: "The Service routes the traffic to targetPort 80 on any Pod matching 'env: DEV'.",
            accent: matchAccent,
            dim: matchDim,
            border: matchBorder,
          },
      ...
      <CodeBlock title="pod.yaml" content={podYamlFull}>
        <PodBlock name="payment-app-1" containerName="pay-image-1" />
        {C("---")}
        {"\n"}
        <PodBlock name="payment-app-2" containerName="pay-image-2" />
        {C("---")}
        {"\n"}
        <PodBlock name="payment-app-3" containerName="pay-image-3" />
        {C("---")}
        {"\n"}
        <PodBlock name="payment-app-4" containerName="pay-image-4" />
      </CodeBlock>
