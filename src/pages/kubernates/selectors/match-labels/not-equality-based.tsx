...
const YamlLine = ({ line, isDark }: { line: string; isDark: boolean }) => {
  const colors = {
    ...
    highlight: isDark ? "#fbbf24" : "#d97706", // orange for negation
  };
  ...
  const renderLine = (text: string) => {
    ...
    return (
      <>
        ...
        <span
          style={{
            color: isNegationKey ? colors.highlight : colors.key,
          }}
        >
          {key}
        </span>
...
// ----------------------------------------------
// Main component – Not‑Equality Label Selector (refactored UI)
// ----------------------------------------------
const NotEqualityBased = () => {
...
spec:
  ...
  // Theme‑aware colour palette (orange/blue accent for negation)
  const bg = isDark ? "#0f0a12" : "#fff7ed";
  ...
  // Primary accent = orange (negation), secondary = blue
  const orange = isDark ? "#fbbf24" : "#d97706";
  const orangeAlpha = isDark ? "rgba(251,191,36,0.08)" : "rgba(217,119,6,0.06)";
  const orangeBorder = isDark ? "rgba(251,191,36,0.28)" : "rgba(217,119,6,0.2)";
  ...
  const headerGrad = isDark
    ...
    : "linear-gradient(135deg,#fff1e6 0%,#ffffff 60%)";

  const mono = "'Space Mono','SF Mono','Menlo',monospace";
  const sans = "'Outfit','Inter',-apple-system,BlinkMacSystemFont,sans-serif";

  // Steps for negation‑based selector usage
  const steps = [
    {
      id: 1,
      title: "Define negation label",
      description:
        "Use the exclamation mark (!) after the label key to mean 'not equal to'.",
      cmd: "selector:\n  matchLabels:\n    app!: zomato-dev",
      accentColor: orange,
      accentBg: orangeAlpha,
      accentBorder: orangeBorder,
    },
    {
      id: 2,
      title: "Apply Deployment",
      description:
        "Kubernetes will select all Pods where app is NOT zomato-dev.",
    ...
    {
      id: 3,
      title: "Verify excluded Pods",
      description:
        "Check the Pods that match the exclusion rule (label missing or different).",
  ...
  const scenarios = [
    {
      ...
      selected: false,
      note: "Excluded (matches the negated value)",
    },
    { label: "app: myapp", selected: true, note: "Different value → selected" },
    {
    ...
    },
    {
      label: "No app label",
      selected: true,
      note: "Missing label → not equal → selected",
    },
  ];
  ...
  // Comparison: equality vs not‑equality
  const comparisons = [
    { feature: "Operator", equality: "=", notEquality: "!=" },
    {
      feature: "Matches when",
      equality: "value is exactly X",
      notEquality: "value is not X",
    },
    {
  ...
  return (
    ...
    >
      <style>{`
        ...
        .neg-pulse       { animation: pulse-glow 2s ease-in-out infinite; }
        .neg-arrow       { animation: arrow-fade 2s ease-in-out infinite; }
        .neg-copy:hover  { color:${orange} !important; border-color:${orangeBorder} !important; }
        .neg-step:hover  { background:${orangeAlpha} !important; transform:translateX(2px); }
        .neg-row:hover   { background:${orangeAlpha} !important; transform:translateX(3px); }
      `}</style>
        ...
        >
          <div
            style={{
              ...
              width: 260,
              height: 260,
              background: `radial-gradient(circle,${orangeAlpha.replace("0.08", "0.22")} 0%,transparent 70%)`,
              pointerEvents: "none",
            }}
          />
          ...
          >
            <div>
              ...
              >
                Not‑equality{" "}
                <span style={{ color: orange }}>Label Selector</span>
              </div>
              ...
              >
                Matches resources where the label value is{" "}
                <strong style={{ color: orange, fontWeight: 500 }}>
                  not equal
                </strong>{" "}
                to the specified value. The <code>!</code> symbol denotes
                negation – useful for exclusion rules and fallback scheduling.
              </div>
            </div>
          ...
          >
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {[
                  {
                    label: "Exclusion operator",
                    bg: orangeAlpha,
                    border: orangeBorder,
                    color: orange,
                  },
                  {
                    label: "Missing label = selected",
                    bg: blueAlpha,
                    border: blueBorder,
                    color: blue,
                  },
                  {
                    label: "Works with matchLabels",
                    bg: purpleAlpha,
                    border: t(
                      "rgba(192,132,252,0.22)",
                      "rgba(147,51,234,0.15)",
            ...
            >
              ...
              <p style={{ fontSize: 12.5, color: txtSec, lineHeight: 1.6 }}>
                The <code>!</code> operator is part of the label key, not a
                separate field. It means <strong>"not equal to"</strong>. If the
                label is missing entirely, the selector also matches (since it
          ...
          >
            ...
            <div>
              ...
              >
                <div style={{ display: "flex", alignItems: "stretch" }}>
                  {(
                    [
                      ...
                      { icon: "🔍", label: "Check\napp!=", accent: true },
                      { icon: "🚫", label: "Not equal?", accent: false },
                    ] as const
                  ).map((step, idx, arr) => (
                    <div
                      key={idx}
                    ...
                    >
                      <div
                        style={{
                          ...
                          border: `1px solid ${step.accent ? orangeBorder : divider}`,
                          borderRadius: 10,
                          padding: "12px 6px",
                        }}
                      >
                        <div
                          style={{
                            fontSize: 20,
                            marginBottom: 7,
                            color: step.accent ? orange : "inherit",
                        ...
                        <div
                          style={{
                            ...
                            fontSize: 9.5,
                            lineHeight: 1.5,
                            color: step.accent ? orange : txtSec,
                            whiteSpace: "pre",
                          }}
                        >
                          {step.label}
                ...
                >
                  <span style={{ color: orange }}>✓</span> Pods where{" "}
                  <code>app</code> is <strong>not</strong>{" "}
                  <code>zomato-dev</code> are selected.
                </div>
              </div>
            </div>
              ...
              >
                {scenarios.map((s, i) => (
                  ...
                  >
                    ...
                    <span
                      style={{
                        background: s.selected ? orangeAlpha : "transparent",
                        color: s.selected ? orange : txtMuted,
                        fontSize: 11,
                        fontWeight: 500,
                        padding: "4px 14px",
                        borderRadius: 24,
            ...
            <div>
              <SectionLabel
                label="Equality vs Not‑equality"
                mono={mono}
                color={txtMuted}
                divider={divider}
              />
            ...
            <div>
              ...
              >
                ...
                <p style={{ fontSize: 13, color: txtSec, lineHeight: 1.6 }}>
                  Not‑equality selectors are great for{" "}
                  <strong>excluding specific values</strong> (e.g., ignore
                  canary Pods, skip legacy versions). Be aware that{" "}
                  <strong style={{ color: orange }}>missing labels</strong> also
                  match – this is often intentional for default/fallback
                  behavior.
                </p>
              </div>
            </div>
...
export default NotEqualityBased;
