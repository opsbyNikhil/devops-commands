// NotEqualityBased.tsx
import React, { useState } from "react";
import { CopyOutlined, CheckOutlined } from "@ant-design/icons";
import { useTheme } from "../../../../Themecontext"; // adjust import

// ----------------------------------------------
// Syntax-highlighting component (unchanged)
// ----------------------------------------------
const YamlLine = ({ line, isDark }: { line: string; isDark: boolean }) => {
  ...
  const renderLine = (text: string) => {
    ...
    // Highlight the "app!" key as special
    ...
    return (
      <>
        {spaces}
        <span style={{ color: isNegationKey ? colors.highlight : colors.key }}>
          {key}
        </span>
        :{value !== "" && <> {coloredValue}</>}
      </>
...
// ----------------------------------------------
// Main component – Not‑equality selector
// ----------------------------------------------
const NotEqualityBased = () => {
  const { isDark } = useTheme();
  const [copied, setCopied] = useState(false);

  // EXACT YAML as you provided (with app!:)
...
spec:
  ...
  const accentOrange = isDark ? "#fbbf24" : "#d97706";
  ...
  // Matrix: which label sets are selected (app! means "not equal")
  const scenarios = [
    {
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Not‑Equality (<span style={{ color: accentOrange }}>app!</span>)
          </div>
          <div
            style={{
              fontSize: "14px",
          ...
          >
            Selects resources where the label <code>app</code> is{" "}
            <strong style={{ color: accentOrange }}>not equal</strong> to{" "}
            <code>zomato-dev</code>. The <code>!</code> symbol denotes negation.
          </div>
        </div>
        ...
        >
          ...
          >
            <div>
              ...
              >
                The exclamation mark <code>!</code> before the colon indicates
                “not equal to”. This selector matches all pods where the label{" "}
                <code>app</code> is <strong>not</strong> <code>zomato-dev</code>
                .
              </p>
              <div
                style={{
              ...
              >
                {["Negation operator", "Exclusion rule", "Simple syntax"].map(
                  (tag) => (
                    <span
                      key={tag}
                      style={{
            ...
            {/* Quick kubectl command (using label selector) */}
            <div
              style={{
              ...
              >
                ...
                >
                  {[
                    ...
                    { title: "Check app!=", icon: "🔍" },
                    { title: "Not equal?", icon: "🚫" },
                  ].map((step, idx) => (
                    <div
                      key={step.title}
                ...
                >
                  <span style={{ color: accentOrange }}>✓</span> Pods where{" "}
                    ...
                    <span
                      style={{
                        background: s.selected
                          ? accentOrangeLight
                          : "transparent",
                        color: s.selected ? accentOrange : textMuted,
                        fontSize: "11px",
                        fontWeight: 500,
                        padding: "4px 14px",
                        borderRadius: "24px",
